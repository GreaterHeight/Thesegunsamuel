...
function render(){
  ...
  box.innerHTML=z.map(n=>`<article class="card course-card"><div><div class="eyebrow">${esc(n.type)}</div><h3>${esc(n.title)}</h3><p>${esc(n.text||"Knowledge node in the SegunSamuel ecosystem.")}</p>${(n.connections||[]).length?`<div class="eyebrow">Connected</div><ul>${n.connections.map(c=>`<li>${esc(c.title)} <small>— ${esc(c.relationship)}</small></li>`).join("")}</ul>`:""}</div>${n.url?`<a class="btn btn-primary" href="${esc(n.url.startsWith('/')?n.url:'/Thesegunsamuel/'+n.url.replace(/^\\//,''))}">Open Node</a>`:`<button class="btn btn-secondary graph-node-connections" type="button" data-node-id="${esc(n.id)}">View Connections</button>`}</article>`).join("")||'<div class="card"><h3>No nodes found.</h3><p>Try another search or choose All layers.</p></div>';
  box.querySelectorAll(".graph-node-connections").forEach(btn=>btn.onclick=()=>{
    const node=z.find(n=>String(n.id)===String(btn.dataset.nodeId)); if(!node)return;
    const existing=btn.parentElement.querySelector(".graph-connections"); if(existing){existing.remove();return;}
    const wrap=document.createElement("div"); wrap.className="graph-connections";
    const c=node.connections||[];
    wrap.innerHTML=c.length?`<div class="eyebrow">Connected</div><ul>${c.map(x=>`<li>${esc(x.title)} <small>— ${esc(x.relationship)}</small></li>`).join("")}</ul>`:"<p>No confirmed outward relationships are currently recorded for this node.</p>";
